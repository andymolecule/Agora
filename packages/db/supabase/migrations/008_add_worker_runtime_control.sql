create table if not exists worker_runtime_control (
...
alter table if exists worker_runtime_control enable row level security;
...
begin
  ...
  if v_active_runtime_version is not null
     and p_runtime_version is distinct from v_active_runtime_version then
    return;
  end if;
