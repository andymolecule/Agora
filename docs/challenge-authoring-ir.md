...
- Support both current managed runtimes and future definition-backed evaluator paths.
...
flowchart LR
    ...
    E -- "Definition-backed evaluator" --> G["Resolve configurable evaluator contract"]
...
type ChallengeAuthoringIR = {
  ...
  evaluation: {
    ...
    evaluator_candidates: Array<{
      id: string;
      kind: "managed_template" | "definition_backed" | "expert";
      confidence: number;
      notes: string[];
    }>;
...
| `routing` | Whether Agora can proceed via managed, definition-backed, or Expert Mode | Makes support boundaries explicit instead of hidden |
...
| `definition_backed` | The challenge is deterministic but needs evaluator configuration beyond today's managed presets | Produce a typed evaluator contract, not a bespoke image by default |
...
- `custom_evaluator`
  ...
  - Expected behavior: route to the definition-backed path or Expert Mode early.

## Managed, Definition-Backed, and Expert Paths
...
2. **Definition-backed evaluators**
...
### Phase 3: Definition-backed evaluator contracts

Status: foundation done, executable expansion in progress

- Definition-backed evaluator contracts now exist between managed templates and full custom/expert paths.
- Typed evaluator archetypes are in place.
- Constrained executable definition-backed paths now exist for structured tables, exact artifact match, and structured record validation.
...
- Expand benchmark-led definition-backed evaluator coverage without adding arbitrary-code execution paths.
...
4. Agora routes the resolved IR into managed, definition-backed, or Expert Mode.
