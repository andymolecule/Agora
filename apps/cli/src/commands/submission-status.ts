...
import { getSubmissionStatusApi, waitForSubmissionStatusApi } from "../lib/api";
import {
  applyConfigToEnv,
  loadCliConfig,
  requireConfigValues,
...
export function buildSubmissionStatusCommand() {
  const cmd = new Command("submission-status")
    ...
    .option("--watch", "Poll until the submission reaches a terminal state", false)
    .option(
      "--interval-seconds <seconds>",
      "Override the polling interval used by --watch",
    )
    .option(
    ...
    .action(
      ...
      while (true) {
        const remainingTimeoutSeconds = Math.max(
          1,
          Math.ceil((timeoutMs - (Date.now() - startedAt)) / 1000),
        );
        ...
        try {
          ...
          if (
            ...
            const fallback = await getSubmissionStatusApi(submissionId);
        ...
        if (Date.now() - startedAt >= timeoutMs) {
          throw new AgoraError(
            "Timed out while waiting for submission progress.",
            {
        ...
        const intervalSeconds =
          overrideIntervalSeconds ?? data.recommendedPollSeconds;
